...
**BULWORK MODE** — the focus-enforcement pillar of the Ledger project-OS. You declare one focus task
(ideally a Ledger project's **Next Action**), start a timed work session, and BULWORK decides — per
...
# With no focus flag, bulwork lists your projects so you can pick one:
...
Or build and use the `bulwork` binary:
...
The other half of Phase 1. Since you can't set a system prompt on claude.ai / Gemini / ChatGPT, BULWORK
...
templates, and the "Ask about BULWORK" help panel. All state is local, human-readable JSON under
...
**Memory-Hub grounding (fast-follow):** set `BULWORK_MEM_BIN` to enrich adjudication with the project's
...
- **Focus task resolution** (`src/ledger.ts`): precedence is `--task` → `--project <id>` → `--last`
  ...
  not-archived), so with none of these `bulwork` lists your projects and asks you to pick — it never
  silently guesses. A chosen project's task is its **Next Action**, falling back to its status note,
  then its name. This is BULWORK's keystone — the focus task *is* the Ledger Next Action, not a string
...
- **Conservative-allow**: a `block` whose confidence is below `BULWORK_MIN_BLOCK_CONFIDENCE`
...
| `BULWORK_PROVIDER` | Force `openrouter` or `anthropic` | auto by key presence |
| `BULWORK_MODEL` | Seed model id (options-page choice overrides it) | provider default |
| `LEDGER_BIN` | Path to the `ledger` CLI | repo path |
| `BULWORK_MIN_BLOCK_CONFIDENCE` | Below this, a block becomes an allow | `0.6` |
| `BULWORK_PORT` | Local service port | `7373` |
| `BULWORK_MEM_BIN` | Memory-Hub `mem` command for grounding (unset = off) | _unset_ |
| `BULWORK_DATA_DIR` | Where all `.data/*` state lives | `bulwork/.data/` |
| `BULWORK_HELP_DIR` | Help-agent corpus directory | `bulwork/help/` |
